/**
 * Convert HTML to Markdown in product JSON files (v2 - Fixed)
 * 
 * This script converts detailedDescription HTML content to clean Markdown
 */

const fs = require('fs');
const path = require('path');

// Simple HTML to Markdown converter - IMPROVED
function htmlToMarkdown(html) {
  ...
  // Remove extra indentation/whitespace at start of lines
  ...
  // Convert simple paragraphs (without complex nested elements)
  md = md.replace(/<p>([^<]*(?:<(?!\/p>|div|table|style)[^<]*)*)<\/p>/gi, (match, content) => {
